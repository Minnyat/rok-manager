...
<svelte:head>
	<title>Versions - Admin</title>
</svelte:head>

<div class="space-y-6">
	<h1 class="text-xl font-bold">{t('av.title')}</h1>
	...
	{#if data.versions.length === 0}
		<div class="card text-center py-6 text-rok-muted text-sm">
			{t('av.noVersions')} <a href="/admin/import" class="text-rok-accent underline">{t('av.importLink')}</a>
		</div>
	{:else}
		<div class="space-y-3">
				...
				<div class="card flex items-center justify-between">
					...
					{#if !v.is_active}
						<form method="POST" action="?/activate" use:enhance>
							...
							<button type="submit" class="btn-primary text-xs">{t('av.activate')}</button>
						</form>
					{/if}
				</div>
			{/each}
