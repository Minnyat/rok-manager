<script lang="ts">
	...
	function statusBadge(status: string) {
		switch (status) {
			case 'active': return { class: 'bg-green-500/20 text-green-400', label: 'Active' };
			case 'draft': return { class: 'bg-yellow-500/20 text-yellow-400', label: 'Draft' };
			case 'archived': return { class: 'bg-gray-500/20 text-gray-400', label: 'Archived' };
...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		<h1 class="text-xl font-bold">Quản lý KvK</h1>
		<button class="btn-primary text-sm" onclick={() => (showCreate = !showCreate)}>
			{showCreate ? '✕ Đóng' : '+ Tạo KvK mới'}
		</button>
	</div>
	...
	{/if}
		...
		<div class="bg-green-500/10 border border-green-500/30 text-green-400 text-sm rounded-lg px-3 py-2">
			Đã tạo KvK thành công! <a href="/admin/kvks/{form.slug}" class="underline text-rok-accent">Xem chi tiết →</a>
		</div>
	{/if}

	{#if showCreate}
		<div class="card">
			<h2 class="text-sm font-medium text-rok-muted mb-3">Tạo KvK mới</h2>
			<form method="POST" action="?/create" use:enhance={() => {
				...
				<div class="space-y-3">
					<div>
						<label for="kvk-name" class="text-sm text-rok-text block mb-1">Tên KvK *</label>
						<input
							id="kvk-name"
							name="name"
							type="text"
							placeholder="VD: KvK S5, KvK 2026-07"
							class="input w-full"
							required
						/>
					</div>
					<div>
						<label for="kvk-desc" class="text-sm text-rok-text block mb-1">Mô tả</label>
						<textarea
							id="kvk-desc"
							name="description"
							placeholder="Mô tả ngắn về KvK này..."
							class="input w-full h-20 resize-none"
						></textarea>
					</div>
					<button type="submit" class="btn-primary" disabled={creating}>
						{creating ? 'Đang tạo...' : 'Tạo KvK'}
					</button>
				</div>
			</form>
		</div>
	...
	{#if data.kvks.length === 0}
		<div class="card text-center py-8 text-rok-muted text-sm">
			Chưa có KvK nào. Nhấn "Tạo KvK mới" để bắt đầu.
		</div>
	{:else}
			...
			{#each data.kvks as kvk, i}
				...
				>
						...
						<div class="flex-1 min-w-0">
							<div class="flex items-center gap-2 mb-1">
								...
								{#if kvk.slug === 'legacy-current'}
									<span class="text-xs px-2 py-0.5 rounded-full bg-rok-accent/20 text-rok-accent">Legacy</span>
								{/if}
							</div>
							{#if kvk.description}
					...
					<div class="mt-2 text-xs text-rok-dim">
						Tạo: {formatDate(kvk.created_at)}
						{#if stats.lastImport}
							· Import gần nhất: {formatDate(stats.lastImport)}
						{/if}
						{#if kvk.active_version_id}
							· <span class="text-rok-accent">Đang có active version</span>
						{/if}
					</div>
				</a>
			{/each}
