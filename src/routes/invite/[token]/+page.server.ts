...
export const load: PageServerLoad = async ({ params, platform }) => {
	...
	const user = await db
		...
		.bind(params.token)
		.first<{ id: number; main_governor_id: number; is_active: number; invite_expires_at: number }>();
	...
	if (activeVersion) {
		const player = await db
			.prepare('SELECT governor_name FROM player_data WHERE version_id = ? AND governor_id = ? LIMIT 1')
...
export const actions: Actions = {
	default: async ({ request, params, platform, cookies }) => {
		...
		if (!username || username.length < 3) {
			return fail(400, { error: 'Tên đăng nhập phải có ít nhất 3 ký tự', username });
		}
		...
		const user = await db
			.prepare('SELECT id, invite_expires_at, is_active FROM users WHERE invite_token = ?')
			.bind(params.token)
